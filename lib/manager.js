...
var Manager = function(options) {
  ...
  setInterval(function() { waitpid(-1, true); }, 10000);
};
...
Manager.prototype.deploy = function(configuration, options) {
  ...
  processes.forEach(function(process) {
    this._startStopProcess(process, this._processes[process.id], options);
...
Manager.prototype.kill = function(pid, options) {
  ...
  instance && instance._process.kill(options.signal || 'SIGTERM');
...
Manager.prototype._startStopProcess = function(process, oldProcess, options) {
  ...
  var numToRestart = 0;
  if (options.restart) {
    numToRestart = runningInstances.length - numToKill;
  }
  
  this._log("Updating \""+process.name+"\" (id="+process.id+", instancedToStop="+numToKill+", instancesToStart="+numToRun+", instancesToRestart="+numToRestart+")");
  
  var kill = function(instance) {
  ...
  if (numToRestart) {
    runningInstances.slice(numToKill, numToKill+numToRestart).forEach(function(instance) {
      kill(instance);
      run();
    }.bind(this));
  }
